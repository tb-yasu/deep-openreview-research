...
def fetch_reviews_on_demand(
    ...
    logger.info(f"📚 Review fetch: {len(paper_ids)} papers requested")
    logger.info(f"   └ Cached: {cached_count} | To fetch: {len(missing_ids)}")
    
    if not missing_ids:
        logger.success("✓ All reviews found in cache!")
    ...
    logger.info(f"🔄 Fetching {len(missing_ids)} reviews from OpenReview API...")
    ...
    if new_reviews:
        ...
        logger.success(f"✓ Cached {len(new_reviews)} new reviews (total: {len(cache)})")
    ...
    logger.success(f"✓ Review fetch complete ({elapsed_total:.1f}s)")
